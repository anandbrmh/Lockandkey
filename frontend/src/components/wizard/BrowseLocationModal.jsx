      ...
      >
        <div className="p-4 sm:p-5 border-b border-slate-100 dark:border-slate-800 flex items-start justify-between gap-3">
          <div className="flex items-start gap-2 sm:gap-3 min-w-0 flex-1">
            <div className="h-9 w-9 rounded-xl bg-teal-600 flex items-center justify-center text-white shrink-0"><MapPin className="h-5 w-5" /></div>
            <div className="min-w-0 flex-1">
              <h3 className="font-bold text-sm sm:text-base text-slate-800 dark:text-white leading-tight">Browse Existing Location</h3>
              <p className="text-[11px] sm:text-xs text-slate-500 dark:text-slate-400 leading-snug">Reuse placement photo to save ImageKit upload</p>
            </div>
          </div>
          <button onClick={onClose} className="p-2 rounded-xl hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-500 shrink-0 touch-manipulation"><X className="h-5 w-5" /></button>
        </div>
        ...
        <form onSubmit={handleSearch} className="p-3 sm:p-4 border-b border-slate-100 dark:border-slate-800 flex gap-2">
          <div className="flex-1 relative min-w-0">
            ...
            <input
              ...
              className="w-full pl-9 pr-3 py-2.5 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-sm focus:outline-none focus:ring-2 focus:ring-teal-500 min-w-0"
            />
          </div>
          <button type="submit" className="shrink-0 px-3 sm:px-4 py-2.5 bg-teal-600 hover:bg-teal-700 text-white rounded-xl text-sm font-bold touch-manipulation">Search</button>
        </form>
        ...
        <div className="flex-1 overflow-auto p-3 sm:p-4 space-y-3 bg-slate-50 dark:bg-slate-950/50 overscroll-contain">
          {loadingLocations ? (
            ...
            <div className="py-10 text-center px-4">
              ...
              <p className="text-xs text-slate-400 mt-1 leading-snug">Capture from camera or browse files as usual — it will be saved.</p>
            </div>
          ) : (
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 sm:gap-3">
              {locations.map((loc) => (
                <button
                  ...
                  className="text-left p-3 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 hover:border-teal-300 dark:hover:border-teal-700 hover:shadow-md active:scale-[0.98] transition-all group flex gap-2 sm:gap-3 min-w-0 touch-manipulation"
                >
                  <div className="h-14 w-14 sm:h-16 sm:w-16 rounded-xl overflow-hidden bg-slate-100 dark:bg-slate-800 flex-shrink-0 relative">
                    ...
                    ) : <div className="h-full w-full flex items-center justify-center text-slate-400"><MapPin className="h-5 w-5 sm:h-6 sm:w-6" /></div>}
                  </div>
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-[10px] text-slate-400 flex items-center gap-1 mt-1"><Clock className="h-3 w-3 shrink-0" />used {loc.usageCount}× · {new Date(loc.lastUsedAt).toLocaleDateString()}</p>
                  </div>
                  <Check className="h-4 w-4 text-teal-600 opacity-0 group-hover:opacity-100 group-active:opacity-100 flex-shrink-0 mt-1" />
                </button>
              ))}
            </div>
          )}
        ...
        <div className="p-3 pb-[max(0.75rem,env(safe-area-inset-bottom))] bg-white dark:bg-slate-900 border-t border-slate-100 dark:border-slate-800 text-center">
          <p className="text-[11px] text-slate-500 dark:text-slate-400 leading-snug">If location not found, close and upload as usual.</p>
        </div>
      </motion.div>
    </div>
  );
