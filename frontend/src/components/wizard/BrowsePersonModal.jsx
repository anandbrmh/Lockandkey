      ...
      >
        <div className="p-5 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <div>
              <h3 className="font-bold text-slate-800 dark:text-white">Browse Existing Person</h3>
              <p className="text-xs text-slate-500 dark:text-slate-400">Reuse handover photo to save ImageKit upload</p>
            </div>
          </div>
        ...
        <div className="flex-1 overflow-auto p-4 space-y-3 bg-slate-50 dark:bg-slate-950/50">
          {loadingPersons ? (
            ...
            <div className="py-10 text-center">
              ...
              <p className="text-sm font-semibold text-slate-600 dark:text-slate-300">No saved persons found</p>
              <p className="text-xs text-slate-400 mt-1">Upload from mobile/drive as usual — it will be saved for next time.</p>
            </div>
          ) : (
              ...
              {persons.map((p) => (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-[10px] text-slate-400 flex items-center gap-1 mt-1"><Clock className="h-3 w-3" />{new Date(p.lastUsedAt).toLocaleDateString()}</p>
                  </div>
