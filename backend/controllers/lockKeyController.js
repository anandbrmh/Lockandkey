...
export const createRecord = async (req, res, next) => {
  try {
    ...
    const [lockPhoto, keyPhoto, placementPhoto, handoverPhoto] = await Promise.all([
    ...
    // System-auto handoverAt: no client date accepted; server time when handover photo uploaded
    const now = new Date();
    ...
    const doc = await LockKeyRecord.create({
      ...
      handoverPerson: {
        name: handoverName,
        role: handoverRole || undefined,
        contactNumber: handoverContact || undefined,
      },
      location: lat || lng ? { lat: lat ? Number(lat) : undefined, lng: lng ? Number(lng) : undefined } : undefined,
...
export const updateRecord = async (req, res, next) => {
  try {
    ...
    const replacePhoto = async (field, folder) => {
      ...
      // delete old from ImageKit if exists — safe delete avoids breaking shared photos
