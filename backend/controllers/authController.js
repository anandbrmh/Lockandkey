...
export const getMe = async (req, res, next) => {
  try {
    // Return fresh user including adminCode for admin
...
// GET /api/auth/admin/settings — admin details
...
// PUT /api/auth/admin/settings — update admin details & 4-digit code (plain, not hashed)
  ...
  try {
    ...
    if (adminCode !== undefined) {
      ...
      if (codeStr === "" || codeStr === "null") {
        ...
        if (!/^\d{4}$/.test(codeStr)) return res.status(400).json({ success: false, message: "adminCode must be exactly 4 digits" });
        admin.adminCode = codeStr; // plain, not hashed
      }
    }
    await admin.save();
