...
// GET /api/lock-key-records/:id — admin can view own+subadmin, not peer admin
export const getRecord = async (req, res, next) => {
  try {
    const filter = await getAdminSingleFilter(req.params.id, req.user);
    const record = await LockKeyRecord.findOne(filter).populate("ownerId", "name email role");
...
// PATCH /api/lock-key-records/:id — owner or admin (merged: admin can edit own+subadmin)
...
// PATCH /api/lock-key-records/:id/person-photo/:personIndex — owner or admin (own+subadmin)
...
export const deleteRecord = async (req, res, next) => {
  try {
    let baseFilter;
    if (isAdmin(req.user)) {
      baseFilter = await getAdminSingleFilter(req.params.id, req.user);
    } else {
      baseFilter = { _id: req.params.id, isDeleted: false, $or: [{ ownerId: req.user._id }, { createdBy: req.user._id }] };
    }
...
// GET /api/lock-key-records/my-assignments — keys/locks assigned to current staff/subadmin via handoverPersons
export const getMyAssignedRecords = async (req, res, next) => {
  try {
    const allowedRoles = ["staff", "subadmin"];
    ...
    const staff = await StaffModel.findOne({ user: req.user._id }).lean();
    if (!staff) {
...
// GET /api/lock-key-records/my-assignments/stats — stats for assigned keys/locks
  ...
  try {
    ...
    const [totalAssignedLocks, active, returned, lost, assignedKeysAgg] = await Promise.all([
      ...
      LockKeyRecord.aggregate([
        ...
        { $match: { $or: [{ "handoverPersons.personId": staff._id }, { "handoverPersons.name": { $regex: `^${escapedName}$`, $options: "i" } }] } },
